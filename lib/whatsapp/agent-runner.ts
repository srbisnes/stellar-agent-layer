/**
 * Server-side agent runner for WhatsApp Business Cloud API.
 */
...
const WA_SYSTEM = `You are the Stellar Agent Layer on WhatsApp — a real autonomous agent on Stellar Testnet.
...
- Create payment INTENTS only — real transfer requires the user to press the Confirm button on WhatsApp
- Show balances and history

## Safety
1. NEVER say a payment was sent unless the user confirmed and the tool returned success.
2. When user wants to send money → always create_payment_intent. Then tell them to press Confirmar pago.
...
- Short messages work best on WhatsApp.
...
export async function runWhatsAppAgent(
  session: WaSession,
  userText: string
): Promise<{ reply: string; paymentIntentId?: string }> {
  ...
  const { text } = await generateText({
    ...
    tools: {
      ...
      get_balance: tool({
        ...
        parameters: z.object({}),
        execute: async () => {
          if (!session.publicKey) return { success: false, error: "No hay wallet." };
      ...
      add_contact: tool({
        ...
        execute: async ({ name, publicKey, note }) => {
          if (!publicKey.startsWith("G") || publicKey.length !== 56) {
            return {
              success: false,
              error: "Public key inválida (debe empezar con G y tener 56 chars).",
            };
          }
      ...
      create_payment_intent: tool({
        description:
          "Create a payment INTENT only. Does NOT send funds. User must confirm with the WhatsApp button.",
        parameters: z.object({
        ...
        execute: async ({ destination, amount, memo }) => {
          try {
            ...
            return {
              ...
              message:
                "Intent creado. El usuario debe presionar Confirmar pago en WhatsApp.",
            };
          } catch (e: any) {
      ...
      get_history: tool({
        ...
        parameters: z.object({ limit: z.number().optional().default(8) }),
        execute: async ({ limit }) => {
